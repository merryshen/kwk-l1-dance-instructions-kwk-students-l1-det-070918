# Code your methods below

def starting_stance
  puts "Plant legs far apart, bend kneew slighty and keep posture loose"
end

def base_footwork
  puts "Lift right foot"
  puts "Return right foot to the ground"
  skip_step("left foot")
end

def skip_step(starting_hand)
  puts "lower #{starting_hand} to the ground"
  puts "bounce #{starting_hand} foot back up slightly, kicking it a few inches back"
  puts "Briefly raise the oppposite foot, then the #{starting_hand} twice more!"
end


def bob_the_reins
  puts "Holding your arms out in front of you, straight and at chest level"
  puts "Cross your righ wrist over your left and hold them together"
  puts "Lift your arm up and down in a loose bouncing movement, in time with the beat of the song. This movement is repeated eight times"
  puts "Do this 7 more times!"
end

def lasso 
  puts"Start arm movement by holding left arm so the back of your wrist is near your chin, pointing your left elbow straight left."
  puts "Lift right arm up and point right elbow diagonally to the right."
  puts "Raise right forearm so that it points straight up, and whip it in small circles to the beat of the song, as if you were a cowboy holding a rope lasso. This movement is repeated eight times as well."
  puts "Do this 7 more times"
end

def bust_a_move
  puts "be cool and be snazzy ;)"
end

def gangnam_style 
  starting_stance
  base_footwork
  skip_step("right foot")
  bob_the_reins
  skip_step("left foot")
  lasso
  bust_a_move
end